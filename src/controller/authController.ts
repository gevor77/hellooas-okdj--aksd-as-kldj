...
import type {LoginType, RequestLogin, ResponseLogin, ResponseGetUsers} from '../type/LoginType.js';

const secretToken = 'SECRET TOKEN';
const activeSessions = {};

const generateAccsessToken = (sessionId: string, id: string): string => {
...
class AuthController {
	...
	// eslint-disable-next-line class-methods-use-this
	async login(req: RequestLogin, res: ResponseLogin) {
		try {
			const {username, password} = req.body;
			const user:LoginType = await Login.findOne({username});
			...
			// eslint-disable-next-line no-underscore-dangle
			const message: string = generateAccsessToken(sessionId, user._id);
	...
	// eslint-disable-next-line class-methods-use-this
	async getUsers(req, res:ResponseGetUsers) {
		try {
			const users:string[] = await Login.find();
